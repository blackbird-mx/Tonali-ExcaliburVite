...
- **`main.ts`** — Engine bootstrap only. Registers scenes (`start` → `Nivel`, `barajaCompleta` → `BarajaCompleta`) and starts the loader. Keep minimal.
...
- **`nivel.ts`** — Main gameplay scene. Manages hand UI, score, buttons ("Jugar Mano", "Descartar", "Mano Aleatoria"), and fire particle effects. Buttons are raw `Actor`s with child `Label`s, not a UI framework.
- **`BarajaCompleta.ts`** — Gallery scene showing all 52 cards in a grid. Navigated via `engine.goToScene('barajaCompleta')`.
...
- **Card selection**: Clicking a card toggles `selected`, moves it up 100px via eased action, and tints the background sprite. Animations pause while selected.
